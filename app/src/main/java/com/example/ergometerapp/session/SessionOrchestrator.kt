...
import com.example.ergometerapp.AppFailureStringResolver
import com.example.ergometerapp.AppFailureUserMessageMapper
...
import com.example.ergometerapp.SessionStartStopPort
import com.example.ergometerapp.ble.FtmsBleClient
import com.example.ergometerapp.ble.FtmsCommandLifecycleEvent
import com.example.ergometerapp.ble.FtmsCommandLifecycleStage
...
import com.example.ergometerapp.ftms.IndoorBikeParseFailure
import com.example.ergometerapp.logging.AppLog
import com.example.ergometerapp.session.diagnostics.SessionDiagnosticsBuffer
import com.example.ergometerapp.session.diagnostics.SessionDiagnosticsEvent
import com.example.ergometerapp.session.release.ReleaseContext
import com.example.ergometerapp.session.release.ReleaseIntent
import com.example.ergometerapp.session.release.ReleaseRampDecider
import com.example.ergometerapp.session.release.ReleaseRampDecision
import com.example.ergometerapp.session.release.ReleaseRampPlan
import com.example.ergometerapp.session.release.ReleaseRampTraceEmitter
import com.example.ergometerapp.session.release.ReleaseRampTraceEvent
import com.example.ergometerapp.session.release.TrainerReleasePolicy
import com.example.ergometerapp.session.release.TrainerControlAuthority
import com.example.ergometerapp.session.release.TrainerReleaseProfiles
...
import com.example.ergometerapp.workout.WorkoutTotalDurationCalculator
...
import com.example.ergometerapp.workout.runner.ImportedHrRuntimeControllerV1
import com.example.ergometerapp.workout.runner.ImportedHrRuntimeEvent
import com.example.ergometerapp.workout.runner.ImportedHrRuntimeState
import com.example.ergometerapp.workout.runner.ImportedHrRuntimeUnreachableTargetStatus
...
class SessionOrchestrator internal constructor(
    ...
    fun onStarterWorkoutSelected() {
        onBundledWorkoutSelected(
            assetPath = com.example.ergometerapp.workout.StarterWorkoutProvider.assetPath,
            sourceName = com.example.ergometerapp.workout.StarterWorkoutProvider.sourceName,
        )
    }
    ...
    fun onBundledWorkoutSelected(
        assetPath: String,
        sourceName: String = com.example.ergometerapp.workout.BundledWorkoutAssetCatalog.fileNameFromAssetPath(assetPath),
    ) {
        val content = try {
            com.example.ergometerapp.workout.BundledWorkoutAssetCatalog.loadText(
                context = context,
                assetPath = assetPath,
            )
        } catch (e: Exception) {
                    ...
                    if (bundledWorkout == null) {
                        applyBundledWorkoutFailure(
                            failure = AppFailureFactory.workoutImportFailure(
                                com.example.ergometerapp.workout.WorkoutImportError(
                                    code = com.example.ergometerapp.workout.WorkoutImportErrorCode.PARSE_FAILED,
    ...
    internal fun forceResetToCleanMenuForDebug() {
        ...
        stopMockTrainerEngine()
        workoutRunner = null
        uiState.runner.value = com.example.ergometerapp.workout.runner.RunnerState.stopped()
        uiState.workoutReady.value = false
        uiState.pendingSessionStartAfterPermission = false
    ...
    private fun scheduleIndoorBikeParsing(payload: ByteArray, generation: Int) {
        ...
        val scheduled = indoorBikeParsingDispatcher.dispatch {
            ...
            val parsedData = com.example.ergometerapp.ftms.parseIndoorBikeData(snapshot) { failure ->
     ...
     */
    private fun syncImportedHrRuntimeForRunnerState(state: com.example.ergometerapp.workout.runner.RunnerState) {
        val selectedImportedWorkout = uiState.selectedImportedWorkout.value
        if (selectedImportedWorkout == null) {
            clearImportedHrRuntimeState()
            return
    ...
    private fun currentSessionLifecycleState(): com.example.ergometerapp.SessionLifecycleState {
        val runtimeState = uiState.sessionRuntimeUiState
        val recoveryState = uiState.connectionRecoveryUiState
        return when {
            runtimeState.stopFlowState.value == com.example.ergometerapp.StopFlowState.STOPPING_AWAIT_ACK ||
                uiState.screen.value == AppScreen.STOPPING -> com.example.ergometerapp.SessionLifecycleState.STOPPING

            uiState.screen.value == AppScreen.SUMMARY &&
                uiState.summary.value != null -> com.example.ergometerapp.SessionLifecycleState.COMPLETED

            !recoveryState.connectionIssueMessageState.value.isNullOrBlank() ->
                com.example.ergometerapp.SessionLifecycleState.FAILED

            runtimeState.pendingSessionStartAfterPermission ->
                com.example.ergometerapp.SessionLifecycleState.PREPARING

            uiState.screen.value == AppScreen.CONNECTING &&
                runtimeState.ftmsReady.value &&
                !runtimeState.ftmsControlGranted.value ->
                com.example.ergometerapp.SessionLifecycleState.AWAITING_CONTROL

            uiState.screen.value == AppScreen.CONNECTING ->
                com.example.ergometerapp.SessionLifecycleState.CONNECTING

            uiState.screen.value == AppScreen.SESSION &&
                runtimeState.postWorkoutFreerideModeActive ->
                com.example.ergometerapp.SessionLifecycleState.RUNNING
            ...
            uiState.screen.value == AppScreen.SESSION &&
                ...
                com.example.ergometerapp.SessionLifecycleState.PAUSED

            uiState.screen.value == AppScreen.SESSION ->
                com.example.ergometerapp.SessionLifecycleState.RUNNING

            else -> com.example.ergometerapp.SessionLifecycleState.IDLE
        }
    }
}
